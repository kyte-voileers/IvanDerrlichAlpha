...
const Project = ({clickHandler, properties}) =>  {
  ...
  fetch(fetchURL)
      ...
      .then(read => {
          ...
          */
          ...
          // console.log(`#description-${repoNameURL}`)
          const descriptionElement = document.querySelector(`#description-${repoNameURL}`)
  ...
  return (
    <div className="container">      
      <img src={imageURL} style={{width:imageWidth}} alt={"Project"}/>
      <div className={`description ${textLeft ? 'reverse' : ''}`} >
        ...
        <a
          ...
          href={deployURL} className="link">
          Live Version
        </a>
      </div>
      <div 
        id={markdownId}
        className='markdown-insertion'
      />
    </div>
  );
}
